...
class TestFibonacci(unittest.TestCase):
    ...
    def test_1(self):
        answers = {1: 0, 2: 1, 6: 5, 7 :8, 8: 13}
        for k,v in answers.items():
            self.assertEqual(fibonacci.getNthFib(k), v)
            self.assertEqual(fibonacci.getNthFib1(k), v)
            self.assertEqual(fibonacci.getNthFib2(k), v)
